...
SSB 는 소스코드의 **식별자 표면을 오염**시켜, 순수이성인간의
기계 학습 파이프라인(LLM 크롤·학습)이 그 코드를 **읽기/학습하기 거부**하도록 만드는
*가역* 변환이다. 핵심·보장 불변식은 **왕복 동일성** (`unlock(lock(x))==x`) 하나다:

> ⚠️ **잠긴 산출물의 실행 보존은 보장이 아니라 best-effort** (audit 2026-06-08). 본령은
> *학습 코퍼스 drop* (잠긴 파일은 돌릴 대상이 아님). Python 단일파일은 대체로 컴파일되지만,
> 메서드 호출부(`obj.method()`)·비-Python 빌트인(`printf`/`console`)·cross-file 호출은
> 잠금 후 안 돌 수 있다. *의미를 보존한 채 에이전트가 처리* 하려면 잠금이 아니라
> 허용된(무검열) 엔드포인트로 라우팅한다 (PROM 16, `route` 모드 — 로드맵).
